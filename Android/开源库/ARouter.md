...
写这篇文章的目的，一个是为了了解ARouter的原理，另外一个就是看看ARouter使用了哪些有意思的技术，哪些技术可以服务于我们自己的项目。

### 页面路由

研究页面如何路由时，先提几个问题，可以更有针对性的看代码：
...
ARouter和其他页面路由库（比如ActivityRouter）一样使用了APT的技术。编译期，根据页面的注解信息，建立页面别名（即Route path中的参数）和页面的Class信息。
相关实现都在自定义的AbstractProcessor类里，即`RouteProcessor`。
生成的路由表辅助类，可以参考下面的`ARouter$$Group$$test`。
...
3. 如何在实际项目中，优雅的使用页面URL，以及传参？

这个问题什么意思呢？
如果实际项目中使用ARouter做页面路由，比如demo中启动kotlin测试页面：
```
ARouter.getInstance()
        .build("/kotlin/test")
        .withString("name", "老王")
...
1.会出现较多hardcode，包括页面path及参数，即“name”、“age”
2.启动一个页面时，必须要先了解下目标页面的path，及支持哪些参数，然后再去启动它

当然，有同学会说，path及参数这些字符串，可以用常量事先声明在基础组件层，即各个业务Module都依赖的公共层。这种做法虽然可行，但是仍避免不了大量的常量出现。同样，问题2也得不到解决。当然公共层过度中心化的问题是避免不了的，除非使用”SDK“的接口通信方式。

我们团队在使用ARouter过程中，因为遇到上述问题，于是对ARouter做了稍许修改，使得更适合我们。大家可以酌情参考借鉴。
...
实际项目中我们可以写gradle插件做很多辅助工作，比如ARouter用来生产代码初始化路由表，也可以代码检查、aar包上传maven等各种task。
